...
En vous appuyant sur les slides du cours et l'implémentation fournie dans le fichier `min_posix.c`, complétez le corp de la fonction `min_thread()` entre les marqueurs **BLOC_DEBUT** et **BLOC_FIN**  afin d'éffectuer la même tâche que précédemment, c-à-d. une recherche de la valeur minimum dans un tableau; sauf que cette fois-ci, votre boucle doit s'exécuter en parallèle.
...
Comparez le temps d'exécution retourné ici avec la valeur retournée dans l'exércice 1. Votre nouvelle version est-elle plus rapide ou plus lente ? (**Indication** : la version parallélisée devrait être plus rapide si implémentée correctement).
